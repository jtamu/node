...
class WeatherPage extends Component {
  ...
  getWeather(id) {
    ...
    fetch(`http://api.openweathermap.org/data/2.5/weather?id=${id}&APPID=${this.OpenWeatherMapKey}&lang=ja&units=metric`)
      ...
      <MuiThemeProvider>
        <Card style={{margin: 30}}>
          ...
          <CardText style={{position: 'relative'}}>
            <RefreshIndicator status={this.state.loading ? 'loading' : 'hide'}
              top={40} left={100} loadingColor="#f00" />
            <WeatherInformation weather={this.state.weather}
              temperature={this.state.temperature} />
          </CardText>
          <CardActions>
            <PlaceSelector places={this.Places}
              actionSelect={(ix) => this.selectPlace(ix)} />
          </CardActions>
        </Card>
      </MuiThemeProvider>
    )
...
const Title = (props) => (
  <h1>{props.place ? props.place + 'の天気' : '天気予報'}</h1>
)
Title.propTypes = {
...
const WeatherInformation = (props) => (
  <List>
    ...
    <ListItem leftIcon={<TemperatureIcon/>} primaryText={
